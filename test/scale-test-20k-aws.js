...
// Test: Ramp up to 20,000 concurrent connections
export const options = {
  scenarios: {
    scale_test: {
      ...
      startVUs: 0,
      stages: [
        { duration: '30s', target: 2000 },   // Warm up to 2K
        { duration: '30s', target: 5000 },   // 5K
        { duration: '30s', target: 8000 },   // 8K
        { duration: '30s', target: 12000 },  // 12K
        { duration: '30s', target: 16000 },  // 16K
        { duration: '30s', target: 20000 },  // 20K
        { duration: '2m', target: 20000 },   // Hold 20K for 2 min
...
export function setup() {
  // Login once to get a valid token for all VUs
  if (TEST_TOKEN) {
  ...
  const email = 'loadtest20k@test.com';
  const password = 'test123';

  http.post(`${BASE_URL}/users/register`, JSON.stringify({
    email: email,
    username: 'loadtest20k',
    password: password,
  ...
  const res = ws.connect(WS_URL, {}, function (socket) {
    ...
    socket.on('message', (data) => {
      ...
      if (msg.type === 'new_message' && msgSendTime > 0) {
        ...
        msgSendTime = 0; // Reset after first measurement
      }
    });
...
export function handleSummary(data) {
  ...
  const summary = `
╔═══════════════════════════════════════════════════════════════╗
║           AWS LOAD TEST RESULTS (Target: 20,000 VUs)          ║
╠═══════════════════════════════════════════════════════════════╣
║ CONNECTIONS                                                   ║
╟───────────────────────────────────────────────────────────────╢
║  Peak Concurrent:     ${String(data.metrics.active_connections?.values?.max || 0).padStart(8)}                            ║
║  Total HTTP Requests: ${String(httpReqs).padStart(8)}                            ║
║  Errors:              ${String(data.metrics.errors?.values?.count || 0).padStart(8)}                            ║
╠═══════════════════════════════════════════════════════════════╣
║ THROUGHPUT                                                    ║
╟───────────────────────────────────────────────────────────────╢
